...
def sync_model( SYNC_I = 30, SYNC_Q = 10, SYNC_U = 10, SYNC_BETA = -3.2 ):
    sync_model = models.SyncPow( amp_I = rj2cmb( 30e9, SYNC_I ), amp_Q = rj2cmb( 30e9, SYNC_Q ), amp_U = rj2cmb( 30e9, SYNC_U ), sync_beta = SYNC_BETA )
    return sync_model
...
## Notice the 'silly' notation [[]] to make it compatible for a default call, 
## since now we deal with distionaries with arrays, contrary to the single pixel case
foreground_dict_default = {
    'cmb':		[ [ 50 ], [ 0.6 ], [ 0.6 ] ],
    'sync':		[ [ 30. ], [ 10. ], [ 10. ], [ -3.2 ] ],
    'freefree':		[ [ 30 ], [ 0 ], [ 0 ], [ -0.118 ] ],
    'mbb':		[ [ 50. ], [ 10 / 1.41 ], [ 10 / 1.41 ], [ 1.6 ], [ 20. ] ], 
    '2mbb_silcar':	[ [ 50 / 1.25 ], [ 10 / 1.41 / 1.25 ], [ 10 / 1.41 / 1.25 ], [ 1.6 ], 
			[ 0.2 ], [ 18. ], [ 22. ], [ 0.25 ], [ 0.25 ], [ 0.25 ] ],
    'hd_fe':		[ [ 50 ], [ 10 / 1.41 ], [ 10 / 1.41 ], [ 1e3 ], [ 1e3 ], [ 0. ] ],
}

# Dictionary of models (see model_values_allsky.py)
def model_dict( fg_dict = foreground_dict_default, idx_px = 0 ):

    model_dict = {
        'cmb':		cmb_model( fg_dict[ 'cmb' ][ 0 ][ idx_px ], fg_dict[ 'cmb' ][ 1 ][ idx_px ], fg_dict[ 'cmb' ][ 2 ][ idx_px ] ), 
        'sync':		sync_model( fg_dict[ 'sync' ][ 0 ][ idx_px ], fg_dict[ 'sync' ][ 1 ][ idx_px ], fg_dict[ 'sync' ][ 2 ][ idx_px ], 
			fg_dict[ 'sync' ][ 3 ][ idx_px ] ), 
        'freefree':	ff_model( fg_dict[ 'freefree' ][ 0 ][ idx_px ], fg_dict[ 'freefree' ][ 1 ][ idx_px ], fg_dict[ 'freefree' ][ 2 ][ idx_px ],
			fg_dict[ 'freefree' ][ 3 ][ idx_px ] ),
        'mbb':		dust_model( fg_dict[ 'mbb' ][ 0 ][ idx_px ], fg_dict[ 'mbb' ][ 1 ][ idx_px ], fg_dict[ 'mbb' ][ 2 ][ idx_px ], 
			fg_dict[ 'mbb' ][ 3 ][ idx_px ], fg_dict[ 'mbb' ][ 4 ][ idx_px ] ), 
        '2mbb_silcar':	two_comp_silcar_model( fg_dict[ '2mbb_silcar' ][ 0 ][ idx_px ], fg_dict[ '2mbb_silcar' ][ 1 ][ idx_px ], fg_dict[ '2mbb_silcar' ][ 2 ][ idx_px ], 
			fg_dict[ '2mbb_silcar' ][ 3 ][ idx_px ], fg_dict[ '2mbb_silcar' ][ 4 ][ idx_px ], fg_dict[ '2mbb_silcar' ][ 5 ][ idx_px ],
			fg_dict[ '2mbb_silcar' ][ 6 ][ idx_px ], fg_dict[ '2mbb_silcar' ][ 7 ][ idx_px ], fg_dict[ '2mbb_silcar' ][ 8 ][ idx_px ], 
			fg_dict[ '2mbb_silcar' ][ 9 ][ idx_px ] ),
	'hd_fe':	hd_fe_model( fg_dict[ 'hd_fe' ][ 0 ][ idx_px ], fg_dict[ 'hd_fe' ][ 1 ][ idx_px ], fg_dict[ 'hd_fe' ][ 2 ][ idx_px ],
			fg_dict[ 'hd_fe' ][ 3 ][ idx_px ], fg_dict[ 'hd_fe' ][ 4 ][ idx_px ], fg_dict[ 'hd_fe' ][ 5 ][ idx_px ] ),
    }
    return model_dict
...
def get_model_param_names( in_list = '' , fit_list = '', Nside = 16, Npix = 0 ):
    ...
    allowed_comps = model_dict( fg_dict = foreground_dict_default, idx_px = 0 )
